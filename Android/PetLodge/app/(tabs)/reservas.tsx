...
//   const [activeStatusTab, setActiveStatusTab] = useState<'Activa' | 'Pendiente' | 'Finalizada'>('Activa');
...
//       ) : reservations.filter(r => r.status_name === activeStatusTab).length === 0 ? (
//         <Text style={styles.emptyText}>No hay reservas {activeStatusTab.toLowerCase()}</Text>
//       ) : (
//         reservations
//           .filter(r => r.status_name === activeStatusTab)
//           .map((item) => (
//             <Pressable
//               key={item.id}
...
//   dropdown: { height: 50, borderColor: '#c0c0c0ff', backgroundColor: '#e4e4e4ff', borderWidth: 1, borderRadius: 8, paddingHorizontal: 10, marginBottom: 12 },
...
const styles = StyleSheet.create({
  ...
  sortDropdown: { height: 50, borderColor: '#c0c0c0ff', backgroundColor: '#f4f4f4', borderWidth: 1, borderRadius: 8, paddingHorizontal: 10, marginBottom: 12, marginHorizontal: 5 },
  checkboxRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 12, paddingVertical: 8, paddingHorizontal: 12 },
  checkboxSelected: { borderRadius: 8 },
  radioOuter: { width: 20, height: 20, borderRadius: 10, borderWidth: 2, borderColor: '#4A3717', alignItems: 'center', justifyContent: 'center', marginRight: 10 },
  radioInner: { width: 10, height: 10, borderRadius: 5, backgroundColor: '#4A3717' },
